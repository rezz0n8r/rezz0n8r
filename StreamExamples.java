...
public class StreamExamples {
	...
	public static List<Widget> getAllWidgets(){
		return Arrays.asList(
				 new Widget("ghost",12),
				 new Widget("vampire",3),
				 new Widget("werewolf" ,9),
	...
	public static List<Widget> howToUseStreamsToFilterAListOfTypesByCondition(List<Widget> unfilteredWidgets) {
		List<Widget> filteredWidgets = unfilteredWidgets.stream()
   ...
   public static Set<Integer> exampleExtractPrimitivePropValListFromObjectsList(List<Widget> widgets){
   ...
   public static List<Widget> objectFilteringByCriteria(List<Widget> unfilteredWidgets){
	   ...
	   List<Widget> filteredWidgets = unfilteredWidgets.stream().filter(widget-> widget.getType() > 0).collect(Collectors.toList());
	   return filteredWidgets;
   }
   
   public static List<Widget> objectFilteringByLambdaReferencingOutsideMethod(List<Widget> unfilteredWidgets){
	   //When passing in a Filtering Method reference, the method must take 1 arg of the Stream-Type, and return a boolean for it: true to include, false to exclude
   ...
   public static void variousExamplesOfSortingStreams() {
	   //when employing the default, built-in comparison (here: Alpha), just use .sorted() in between:
	  ...
	  List<String> reverseSortedListStrs = unsortedStrings.sorted(Comparator.reverseOrder()).collect(Collectors.toList());
