...
public class KDTree2D {
	...
	private Rectangle2D.Float getSubrectangle(final Rectangle2D startingRect, float dividerVal, int dimIndex,
			int whichHalf) {
		if (dimIndex == 0) {
			if (whichHalf == 0) {
				float newWidth = dividerVal - (float) startingRect.getMinX();
				return new Rectangle2D.Float((float) startingRect.getMinX(), (float) startingRect.getMaxY(), newWidth,
						(float) startingRect.getHeight());
			} else {
				float newWidth = (float) startingRect.getMaxX() - dividerVal;
				return new Rectangle2D.Float(dividerVal, (float) startingRect.getMaxY(), newWidth,
						(float) startingRect.getHeight());
			}
		} else {
			if (whichHalf == 0) {
				...
				return new Rectangle2D.Float((float) startingRect.getMinX(), (float) startingRect.getMinY(),
						(float) startingRect.getWidth(), newHeight);
			} else {
				float newHeight = (float) startingRect.getMaxY() - dividerVal;
				return new Rectangle2D.Float((float) startingRect.getMinX(), (float) startingRect.getMaxY(),
						(float) startingRect.getWidth(), newHeight);
			}
		}
