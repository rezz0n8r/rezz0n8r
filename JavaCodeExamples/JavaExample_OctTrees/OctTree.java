...
public class OctTree<K, V> {
	...
	public OctNode<K, V> getSmallestOctantContainingPoint3D(Vector3D point3d) {
		...
		ConvexVolume wholeSpace = root.getRegion();
	 ...
	 * 
	 */
	private OctNode<K, V> findSmallestOctantContainingPointR(OctNode<K, V> focusNode, Vector3D point3d,
			ConvexVolume smallestOctantYet) {
		if (focusNode == null || (!focusNode.containsPoint(point3d))) {
			return null;
		}
		if (focusNode.getRegion().getSize() >= smallestOctantYet.getSize()) {
			/// focus nodes region is Not smaller than the smallest Octant, So exit this
			/// path:
			return null;
		}
		...
		for (Entry<K, OctNode<K, V>> childEntry : childNodes.entrySet()) {
			...
			OctNode<K, V> candidateSmallerSubOctant = findSmallestOctantContainingPointR(childNode, point3d,
					smallestOctantYet);
			if (candidateSmallerSubOctant == null) {
				...
				} else if (candidateSmallerSubOctant.getRegion().getSize() < smallestFound.getRegion().getSize()) {
	...
	public OctNode<K, V> findSmallestOctantEntirelyContainingSearchVolume(ConvexVolume searchVol)
			throws OctTreeException {
		if (searchVol.isEmpty()) {
			return null;
		}
		if (this.root == null) {
			throw new OctTreeException("Cannot search an empty OctTree");
		}
		return findSmallestOctantContainingVolumeR(root, searchVol);
	}

	private OctNode<K, V> findSmallestOctantContainingVolumeR(OctNode<K, V> focusNode, ConvexVolume searchBox) {
		...
		for (Entry<K, OctNode<K, V>> childEntry : childNodes.entrySet()) {
			...
			if (recursiveFind == null) {
				continue; // THIS child octant doesnt contain the box, skip to the next child in this
							// loop...
			} else {
				if (smallestFind == null || recursiveFind.getRegion().getSize() < smallestFind.getRegion().getSize()) {
